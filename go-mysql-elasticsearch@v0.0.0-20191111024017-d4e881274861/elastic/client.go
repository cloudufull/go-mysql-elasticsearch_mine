...
func (r *BulkRequest) Debug_bulk(buf *bytes.Buffer) error {
     return r.bulk(buf)
}
...
func (c *Client) DoBulk(url string, items []*BulkRequest) (*BulkResponse, error) {
	...
	resp, err := c.DoRequest("POST", url, &buf)
        ...
        spend:=end-begin
        log.Infof("== this-bulk spend: %d sec bulk size is : %d",spend,dnum)
	if err != nil {
		return nil, errors.Trace(err)
	}
	...
	if len(data) > 0 {
        ...
        if err != nil {
              log.Errorf(">~~~~~~~~~~~~ error when switch es response to struck using  json.Unmarshal~~~~~~~~<")
              log.Errorf(string(data))
              log.Errorf(">~~~~~~~~~~~~json.Unmarshal~~~~~~~~<")
